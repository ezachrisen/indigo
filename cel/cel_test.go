...
type Rule struct {
	...
	schema   rules.Schema
	rules    []Rule
	expected rules.Result
}

func (r Rule) ID() string           { return r.id }
func (r Rule) Schema() rules.Schema { return r.schema }
func (r Rule) Expression() string   { return r.expr }
func (r Rule) Rules() []rules.Rule {
	rls := make([]rules.Rule, len(r.rules))
...
func TestSimpleCEL(t *testing.T) {
	...
	rule := Rule{
		id:       "student_actions",
		schema:   education,
		expected: rules.Result{Value: true},
		rules: []Rule{
			{
				id:       "honors_student",
				schema:   education,
				expr:     `student.GPA >= 3.6 && student.Status!="Probation" && !("C" in student.Grades)`,
				expected: rules.Result{Value: false},
			},
			{
				id:       "at_risk",
				schema:   education,
				...
				rules: []Rule{
					{
						...
						schema:   education,
						expr:     `2.0+6.0`,
						expected: rules.Result{Value: 8.1},
					},
				},
			},
		},
...
func PrintResults(res *rules.Result, tabs int) {
	...
	fmt.Printf("%-30s %v ", fmt.Sprintf("%s%s", strings.Repeat(" ", tabs), res.RuleID), res.Value)
	r := *res.Rule
	test_rule := r.(Rule)
...
// 				// 		schema:   education,
// 				// 		expr:     `2.0+6.0`,
// 				// 		expected: rules.Result{Float64Value: 8.0},
// 				// 	},
// 				// },
// 			},
// 		},
