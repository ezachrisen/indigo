...
func TestVaultShards(t *testing.T) {
	...
	//--------------------------------------------------------------------------------
	// Create the vault with the root rule.
...
└── woodlawn
	...
	// Let's add a new rule to Woodlawn. Based on the sharding specification, it shold be placed into "woodlawnForeign"

	woodlawnForeignJustOK := indigo.NewRule("woodlawnForeignJustOK", `school =="woodlawn" && class == 2026 && gpa > 2.0 && gpa < 3.7 && nationality != "US"`)

	err = v.Mutate(indigo.Add(woodlawnForeignJustOK, "root"))
	if err != nil {
		t.Fatal(err)
	}
