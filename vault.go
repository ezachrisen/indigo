...
// 	rr := *r
// 	rr.Rules = maps.Clone(r.Rules)
// 	fmt.Printf("shallow copy: %s %p (from %p)\n", rr.ID, &rr, r)
// 	return &rr
// }
//
// // delete removes the rule with the id from the root rule r.
...
// // to the (now) safe parent. If only the parent needs to be made safe,
...
// 			fmt.Println("  made shallow ", parent.ID)
...
// 			fmt.Println("  made shallow child ", childCopy.ID, " added to ", parent.ID)
