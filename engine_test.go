...
func TestParallelEvalLeakOnEarlyError(t *testing.T) {
	...
	_, evalErr := engine.Eval(ctx, rootRule, map[string]any{}, indigo.Parallel(1, 3))
	require.Error(t, evalErr)
	require.Contains(t, evalErr.Error(), "mock error for rule ruleError")

	// Give some time for other goroutines to potentially get stuck or finish
	time.Sleep(mockEval.evalDelay * 3)

	require.Equal(t, int32(0), atomic.LoadInt32(&activeEvals), "Expected no active evaluations after early error")
}
...
func TestParallelEvalLeakOnContextCancel(t *testing.T) {
	...
	require.Equal(t, int32(0), atomic.LoadInt32(&activeEvals), "Expected no active evaluations after context cancellation")
}
...
func TestParallelEvalPanicHandling(t *testing.T) {
	...
	_, evalErr := engine.Eval(ctx, rootRule, map[string]any{}, indigo.Parallel(1, 3))
	require.Error(t, evalErr)
	require.Contains(t, evalErr.Error(), "panic during parallel rule evaluation: mock panic for rule rulePanic")

	time.Sleep(mockEval.evalDelay * 3)
	require.Equal(t, int32(0), atomic.LoadInt32(&activeEvals), "Expected no active evaluations after panic")
}
